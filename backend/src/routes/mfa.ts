...
function cookieOpts() {
  ...
  return { httpOnly: true, secure: prod, sameSite: prod ? 'none' as const : 'lax' as const, path: '/' };
}

export async function mfaRoutes(app: FastifyInstance): Promise<void> {
  // Generate TOTP secret + QR code for setup
  app.post('/api/v1/auth/mfa/setup', { preHandler: [app.requireAuth] }, async (_req, reply) => {
    ...
    // Store secret temporarily (mfa_enabled stays false until verified)
    await db.update(appSettings).set({ mfa_secret: secret.base32 }).where(eq(appSettings.id, 1));
  ...
  app.post('/api/v1/auth/mfa/verify', { preHandler: [app.requireAuth] }, async (req, reply) => {
    ...
    await db.update(appSettings).set({ mfa_enabled: true }).where(eq(appSettings.id, 1));
    return { data: { enabled: true } };
  });

  // Disable MFA
  app.delete('/api/v1/auth/mfa', { preHandler: [app.requireAuth] }, async (_req, _reply) => {
  ...
  app.post('/api/v1/auth/mfa/validate', authRateLimitConfig, async (req, reply) => {
    ...
    // Require mfa_pending cookie set during password-auth step
    ...
    const jti = uuidv4();
    const realToken = app.jwt.sign({ role: 'admin', jti }, { expiresIn: expiry });
    const decoded = app.jwt.decode<{ exp: number }>(realToken);

    void reply
      .clearCookie('petstay_mfa_pending', { path: '/' })
      .setCookie('petstay_token', realToken, { ...cookieOpts(), maxAge: parseExpiry(expiry) });
